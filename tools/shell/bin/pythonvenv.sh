...
function create_env() {
  ...
  local major_version
  major_version=$(echo "$version" | cut -b 1)
  cd && mkdir -p .venv && cd ".venv" || return 1
  ...
  if [ "$major_version" = "2" ]; then
    # for python 2
    if ! "$pythonbin" -m virtualenv -h &>/dev/null; then
      if ! "$pythonbin" -m pip install virtualenv && "$pythonbin" -m virtualenv -h &>/dev/null; then
        echo "Unable to install virtualenv" && return 1
      fi
    fi
  ...
  local env_first_letter
  env_first_letter=$(echo "$env_name" | cut -b 1)
  if [[ "$env_first_letter" == "2" ]] || [[ "$env_first_letter" == "3" ]]; then
    [ -d "$env_first_letter" ] && rm -rf "$env_first_letter" # Delete if it is a folder (symlink not always activated on Windows)
  ...
  if uname -s | grep -E '^(MSYS_NT|MINGW64_NT|CYGWIN_NT)' &>/dev/null || uname -a | grep -ci 'Microsoft' &>/dev/null; then
    ...
    if ! type get_path_to_posix &>/dev/null; then
      if [ -f "$SCRIPT_ROOT/../source/path_windows.sh" ]; then
        # shellcheck source=../source/path_windows.sh
        source "$SCRIPT_ROOT/../source/path_windows.sh"
      else
        local common_env_path
        common_env_path="$(head -1 "$HOME/.common_env_path")"
    ...
    python_env_path="$(grep -E '^VIRTUAL_ENV=' "$pythonactivate" | head -1 | sed -re "s/VIRTUAL_ENV=(['\"])([^'\"]+)['\"]/\2/")"
    if [ "$(printf '%s' "$python_env_path" | grep -c ':')" -ne 0 ]; then
      local python_env_path_posix
      python_env_path_posix="$(get_path_to_posix "$python_env_path")"
